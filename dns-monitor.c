...
const uint8_t *extract_domain_name(const uint8_t *packet, const uint8_t *reader, char *domain_name)
{
    ...
    if (jumped)
    {
        // printf("\n%02x %02x %02x %02x %d\n", orig_reader[0], orig_reader[1], orig_reader[2], orig_reader[3], was_in_else);
...
void print_domains(unsigned short type, unsigned short length, const uint8_t *rdata, const uint8_t *packet, char *domain_name)
{
    ...
    { // SOA record
        ...
        unsigned int serial = ntohl(*(unsigned int *)rdata);
        vdata += 4;
        unsigned int refresh = ntohl(*(unsigned int *)rdata);
        vdata += 4;
        unsigned int retry = ntohl(*(unsigned int *)rdata);
        vdata += 4;
        unsigned int expire = ntohl(*(unsigned int *)rdata);
        vdata += 4;
        unsigned int minimum = ntohl(*(unsigned int *)rdata);
        vdata += 4;
    ...
    { // SRV record
        ...
        unsigned short priority = ntohs(*(unsigned short *)adata);
        adata += 2;
        unsigned short weight = ntohs(*(unsigned short *)adata);
        adata += 2;
        unsigned short port = ntohs(*(unsigned short *)adata);
        adata += 2;
